...
class SongFavorite(APIView):
    ...
    def get(self, request):
        ...
        liked_songs = Usersong.objects.filter(id_user=user, islike=True).values('id_song')
        songs = Song.objects.filter(Q(id__in=liked_songs))
        user_songs = Usersong.objects.filter(id_user='suziikage', islike=True)
        serializerUserSong = UserSongSerializer(user_songs, many=True).data
        serializer = SongForPlaylistSerializer(songs, many=True).data
        res = {
            'songs': serializer,
            'user_songs': serializerUserSong
        }
